...
app.post("/update", async (req: Request, res: Response | any) => {
  const { latitude, longitude, speed, battary, timestamp, username }: Event = req.body;
  const newLocation = { latitude: parseFloat(latitude), longitude: parseFloat(longitude), batt: battary, time: timestamp, s: speed, numberOfAllRecivedEvents };
  console.log(`POST update/${count++} `, req.body);
  console.log("")
  res.status(200).json()
});

async function dbStoreEvent(data: Event) {
...
app.get("/update-data", async (req: Request, res: Response | any) => {
  ...
  // await dbStoreEvent(newLocation)
